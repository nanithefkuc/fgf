...
//! (`kernel::x86::proven`).
//!
//! The in-crate kernel tests (`src/kernel/tests.rs`) call the *selected*
//! entrypoints directly, so the public proven wrappers — geometry
//! validation plus token proof plus dispatch — are never exercised there.
//! These tests drive the wrappers exactly as an external `internals`
//! consumer would: summon a capability token per tier, skip the tier when
//! the host cannot prove it, and compare every observable byte against the
//! portable scalar oracle (`kernel::scalar`) or per-lane field arithmetic.
//!
...
use fgf::{
    FanPaar16, FanPaar32, FanPaar64, Gf8B, Gf8D, Gf16, Gf32, Gf64, Goldilocks, Mersenne31,
    fan_paar, gf8b, gf8d, gf16, gf32, gf64, goldilocks, mersenne31,
};
...
// ---------------------------------------------------------------------------
// Scalar oracles — always the portable reference, never the kernel under test.
// ---------------------------------------------------------------------------
...
fn proven_bytes_xor_matches_scalar() {
    ...
    for &row_len in ROW_LENS {
        for &nrows in &[0usize, 1, 2, 3, 4, 5, 8, 9, 13] {
            ...
            let mut want = got.clone();
            x86::proven::bytes::xor_rows_avx2(v3, &mut got, &src, row_len);
            ...
            let mut want = got.clone();
            x86::proven::bytes::xor_rows_sse2(v2, &mut got, &src, row_len);
            for (d, s) in want.chunks_exact_mut(row_len).zip(src.chunks(row_len)) {
                scalar::xor(d, s);
            }
...
fn proven_bytes_xor_gather_matches_offsets() {
    ...
    for offsets in [
        ...
        let mut want = got.clone();
        x86::proven::bytes::xor_gather_avx2(v3, &region, &mut got, &offsets);
...
fn proven_gf8_mul_kernels_match_scalar() {
    ...
    check_mul_add(
        ...
        &coeffs,
        gf8b_ref,
        |dst, c, src| x86::proven::gf8::mul_add_gfni(v3gfni, dst, c, src),
    );
    check_mul_assign(
        "gf8::mul_assign_gfni",
        LENGTHS8,
        ...
        |dst, c| x86::proven::gf8::mul_assign_gfni(v3gfni, dst, c),
    );
    check_mul_into(
        "gf8::mul_into_gfni",
        LENGTHS8,
        &coeffs,
        gf8b_ref,
        |dst, c, src| x86::proven::gf8::mul_into_gfni(v3gfni, dst, c, src),
    );
    check_mul_add(
        "gf8::mul_add_avx2",
        LENGTHS8,
        &coeffs,
        gf8b_ref,
        |dst, c, src| x86::proven::gf8::mul_add_avx2(v3, dst, scale_table(c), src),
    );
    check_mul_assign(
        "gf8::mul_assign_avx2",
        LENGTHS8,
        &coeffs,
        scalar::mul_assign::<Gf8B>,
        |dst, c| x86::proven::gf8::mul_assign_avx2(v3, dst, scale_table(c)),
    );
    check_mul_into(
        "gf8::mul_into_avx2",
        LENGTHS8,
        &coeffs,
        gf8b_ref,
        |dst, c, src| x86::proven::gf8::mul_into_avx2(v3, dst, scale_table(c), src),
    );
    check_mul_add(
        "gf8::mul_add_ssse3",
        LENGTHS8,
        &coeffs,
        gf8b_ref,
        |dst, c, src| x86::proven::gf8::mul_add_ssse3(v2, dst, scale_table(c), src),
    );
    check_mul_assign(
        ...
        &coeffs,
        scalar::mul_assign::<Gf8B>,
        |dst, c| x86::proven::gf8::mul_assign_ssse3(v2, dst, scale_table(c)),
    );
    check_mul_into(
        "gf8::mul_into_ssse3",
        LENGTHS8,
        &coeffs,
        gf8b_ref,
        |dst, c, src| x86::proven::gf8::mul_into_ssse3(v2, dst, scale_table(c), src),
    );

    check_elementwise::<Gf8B>("gf8::elementwise_gfni", LENGTHS8, |dst, a, b| {
        x86::proven::gf8::elementwise_gfni(v3gfni, dst, a, b)
    });
    check_elementwise::<Gf8B>("gf8::elementwise_avx2", LENGTHS8, |dst, a, b| {
        x86::proven::gf8::elementwise_avx2::<0x1b>(v3, dst, a, b)
    });
    check_elementwise::<Gf8B>("gf8::elementwise_ssse3", LENGTHS8, |dst, a, b| {
        x86::proven::gf8::elementwise_ssse3::<0x1b>(v2, dst, a, b)
    });
}
...
fn proven_gf8d_affine_kernels_match_scalar() {
    ...
    check_mul_add(
        ...
        gf8d_ref,
        |dst, c, src| {
            x86::proven::gf8::mul_add_affine(token, dst, affine_8d(c), scale_table_8d(c), src)
        },
    );
    check_mul_assign(
        ...
        |dst, c| x86::proven::gf8::mul_assign_affine(token, dst, affine_8d(c), scale_table_8d(c)),
    );
    check_mul_into(
        "gf8::mul_into_affine",
        LENGTHS8,
        ...
        gf8d_ref,
        |dst, c, src| {
            x86::proven::gf8::mul_into_affine(token, dst, affine_8d(c), scale_table_8d(c), src)
        },
    );
}
...
fn proven_gf8_scatter_gather_match_scalar() {
    ...
    check_scatter(
        ...
        |rows, row_len, coeffs, src| {
            x86::proven::gf8::scatter_gfni(token, rows, row_len, coeffs, src)
        },
    );
    check_scatter(
        "gf8::scatter_affine",
        ...
        |rows, row_len, coeffs, src| {
            x86::proven::gf8::scatter_affine(token, rows, row_len, coeffs, src)
        },
    );
    ...
    for &row_len in &[32usize, 130, 300] {
        check_gather(
            ...
            |dst, coeffs, srcs| x86::proven::gf8::gather_gfni(token, dst, coeffs, srcs),
        );
        check_gather(
            ...
            |dst, coeffs, srcs| x86::proven::gf8::gather_gfni_axpy_tail(token, dst, coeffs, srcs),
        );
        check_gather(
            ...
            |dst, coeffs, srcs| x86::proven::gf8::gather_gfni_split(token, dst, coeffs, srcs),
        );
        ...
        check_gather(
            ...
            |dst, coeffs, srcs| x86::proven::gf8::gather_gfni_tile::<1>(token, dst, coeffs, srcs),
        );
        check_gather(
            ...
            gf8b_ref,
            |i| gf8b::Elem::from_raw((i as u8).wrapping_mul(47).wrapping_add(11)),
            |dst, coeffs, srcs| x86::proven::gf8::gather_gfni_tile::<2>(token, dst, coeffs, srcs),
        );
        check_gather(
            ...
            gf8b_ref,
            |i| gf8b::Elem::from_raw((i as u8).wrapping_mul(47).wrapping_add(11)),
            |dst, coeffs, srcs| x86::proven::gf8::gather_gfni_tile::<3>(token, dst, coeffs, srcs),
        );
        check_gather(
            ...
            gf8b_ref,
            |i| gf8b::Elem::from_raw((i as u8).wrapping_mul(47).wrapping_add(11)),
            |dst, coeffs, srcs| x86::proven::gf8::gather_gfni_tile::<4>(token, dst, coeffs, srcs),
        );
        check_gather(
            "gf8::gather_affine",
            row_len,
            ...
            |dst, coeffs, srcs| x86::proven::gf8::gather_affine(token, dst, coeffs, srcs),
        );
        ...
        for &nterms in NTERMS {
            ...
            let factors: Vec<_> = coeffs
                .iter()
                .map(|&c| x86::proven::prepare_affine_8b(c))
                .collect();
            let mut got = noise(row_len, 0xd1);
            let mut want = got.clone();
            x86::proven::gf8::gather_affine_8b(token, &mut got, &factors, &srcs);
            for (&coeff, &src) in coeffs.iter().zip(&srcs) {
                gf8b_ref(&mut want, coeff, src);
            }
            assert_eq!(got, want, "gf8::gather_affine_8b: terms {nterms}");
        }
    }
}
...
fn proven_gf8_matrix_kernels_match_scalar() {
    ...
    check_matrix_accumulate(
        ...
        |rows, row_len, nrows, terms| {
            x86::proven::gf8::matrix_gfni(token, rows, row_len, nrows, terms)
        },
    );
    check_matrix_accumulate(
        "gf8::matrix_affine",
        ...
        gf8d_ref,
        |rows, row_len, nrows, terms| {
            x86::proven::gf8::matrix_affine(token, rows, row_len, nrows, terms)
        },
    );
    check_matrix_overwrite(
        "gf8::matrix_overwrite_gfni",
        ROW_LENS,
        gf8b_coeff_at2,
        gf8b_ref,
        |rows, row_len, nrows, terms| {
            x86::proven::gf8::matrix_overwrite_gfni(token, rows, row_len, nrows, terms)
        },
    );
    check_matrix_overwrite(
        "gf8::matrix_overwrite_affine",
        ROW_LENS,
        gf8d_coeff_at2,
        gf8d_ref,
        |rows, row_len, nrows, terms| {
            x86::proven::gf8::matrix_overwrite_affine(token, rows, row_len, nrows, terms)
        },
    );
    ...
    check_matrix_accumulate(
        "gf8::matrix_gfni_with",
        ROW_LENS,
        gf8b_coeff_at2,
        gf8b_ref,
        |rows, row_len, nrows, terms| {
            flat_gf8b(terms, nrows, |matrix| {
                // SAFETY: FlatMatrix borrows immutable slices for the whole
                // call, so `len`, `coefficient`, and `source` cannot change.
                unsafe { x86::proven::gf8::matrix_gfni_with(token, rows, row_len, nrows, matrix) }
            });
        },
    );
    check_matrix_overwrite(
        ...
        |rows, row_len, nrows, terms| {
            flat_gf8b(terms, nrows, |matrix| {
                // SAFETY: as above — stable borrowed-slice provider.
                unsafe {
                    x86::proven::gf8::matrix_overwrite_gfni_with(
    ...
    check_matrix_accumulate(
        ...
        |rows, row_len, nrows, terms| {
            flat_gf8d(terms, nrows, |matrix| {
                // SAFETY: as above — stable borrowed-slice provider.
                unsafe { x86::proven::gf8::matrix_affine_with(token, rows, row_len, nrows, matrix) }
            });
        },
    );
    check_matrix_overwrite(
        ...
        |rows, row_len, nrows, terms| {
            flat_gf8d(terms, nrows, |matrix| {
                // SAFETY: as above — stable borrowed-slice provider.
                unsafe {
                    x86::proven::gf8::matrix_overwrite_affine_with(
                        token, rows, row_len, nrows, matrix,
                    )
                }
            });
...
fn scattered_case_gf8b(token: X64V3GfniCryptoToken, row_len: usize, starts: &[usize]) {
    ...
    let mut want = got.clone();
    x86::proven::gf8::matrix_scattered_gfni(token, &mut got, row_len, starts, &terms);
    for &(coeffs, src) in &terms {
        for (&start, &coeff) in starts.iter().zip(coeffs) {
            gf8b_ref(&mut want[start..start + row_len], coeff, src);
        }
    }
    assert_eq!(got, want, "matrix_scattered_gfni: row_len {row_len}");
}
...
fn scattered_case_gf8d(token: X64V3GfniCryptoToken, row_len: usize, starts: &[usize]) {
    ...
    let mut want = got.clone();
    x86::proven::gf8::matrix_scattered_affine(token, &mut got, row_len, starts, &terms);
    for &(coeffs, src) in &terms {
        for (&start, &coeff) in starts.iter().zip(coeffs) {
            gf8d_ref(&mut want[start..start + row_len], coeff, src);
        }
    }
    assert_eq!(got, want, "matrix_scattered_affine: row_len {row_len}");
}
...
fn proven_overwrite_wrappers_zero_addressed_rows_on_empty_terms() {
    ...
    let mut want = got.clone();
    x86::proven::gf8::matrix_overwrite_gfni(token, &mut got, row_len, nrows, &[]);
    want[..nrows * row_len].fill(0);
    assert_eq!(got, want, "matrix_overwrite_gfni: empty terms");

    let mut got = noise(nrows * row_len + surplus, 0xf2);
    let mut want = got.clone();
    x86::proven::gf8::matrix_overwrite_affine(token, &mut got, row_len, nrows, &[]);
    want[..nrows * row_len].fill(0);
    assert_eq!(got, want, "matrix_overwrite_affine: empty terms");
    ...
    let mut want = got.clone();
    // SAFETY: empty stable borrowed-slice provider.
    unsafe {
        x86::proven::gf8::matrix_overwrite_gfni_with(token, &mut got, row_len, nrows, &matrix)
    }
    want[..nrows * row_len].fill(0);
    assert_eq!(got, want, "matrix_overwrite_gfni_with: empty terms");
    ...
    // SAFETY: empty stable borrowed-slice provider.
    unsafe {
        x86::proven::gf8::matrix_overwrite_affine_with(token, &mut got, row_len, nrows, &matrix)
    }
    want[..nrows * row_len].fill(0);
    assert_eq!(got, want, "matrix_overwrite_affine_with: empty terms");

    // Accumulation with no terms is a no-op: nothing changes.
    let mut got = noise(nrows * row_len + surplus, 0xf5);
    let want = got.clone();
    x86::proven::gf8::matrix_gfni(token, &mut got, row_len, nrows, &[]);
    assert_eq!(got, want, "matrix_gfni: empty terms is a no-op");
    ...
    x86::proven::gf8::matrix_gfni(token, &mut got, row_len, 0, &terms);
    x86::proven::gf8::matrix_overwrite_gfni(token, &mut got, row_len, 0, &terms);
    ...
    x86::proven::gf8::scatter_gfni(token, &mut got, 0, &coeffs, &[]);
    x86::proven::gf8::gather_gfni(token, &mut got, &[], &[]);
...
fn proven_gf16_degenerate_boundaries() {
    ...
    let want = got.clone();
    x86::proven::gf16::matrix_gfni(token, &mut got, row_len, nrows, &[]);
    assert_eq!(got, want, "gf16::matrix_gfni: empty terms is a no-op");

    // Zero rows and empty gather sources.
    let mut got = noise(row_len + surplus, 0x172);
    let want = got.clone();
    let coeffs = [gf16::Elem::from_raw(0xbeef)];
    x86::proven::gf16::gather_gfni(token, &mut got, &[], &[]);
    x86::proven::gf16::scatter_gfni(token, &mut got, 0, &coeffs, &[]);
...
#[test]
#[should_panic(expected = "gf8::mul_add_gfni")]
fn proven_gf8_mul_add_rejects_length_mismatch() {
    ...
    let src = vec![0u8; 17];
    x86::proven::gf8::mul_add_gfni(token, &mut dst, gf8b::Elem::from_raw(3), &src);
}

#[test]
#[should_panic(expected = "not a whole number of 2-byte elements")]
...
#[test]
#[should_panic(expected = "rows is 16 bytes but 2 rows of 16 bytes need 32")]
fn proven_scatter_rejects_short_rows_buffer() {
    ...
    x86::proven::gf8::scatter_gfni(token, &mut rows, 16, &coeffs, &src);
}

#[test]
#[should_panic(expected = "term 0 coefficients")]
fn proven_matrix_rejects_term_coefficient_mismatch() {
    ...
    x86::proven::gf8::matrix_gfni(token, &mut rows, 32, 2, &terms);
}
...
fn proven_matrix_scattered_rejects_overlapping_rows() {
    ...
    x86::proven::gf8::matrix_scattered_gfni(token, &mut dst, 64, &starts, &terms);
}
...
fn proven_gather_rejects_count_mismatch() {
    ...
    x86::proven::gf8::gather_gfni(token, &mut dst, &coeffs, &srcs);
}

#[test]
#[should_panic(expected = "partial trailing row")]
fn proven_xor_rows_rejects_partial_row() {
    ...
    let src = vec![0u8; 33];
    x86::proven::bytes::xor_rows_avx2(v3, &mut dst, &src, 16);
}
...
fn proven_xor_rows_rejects_zero_row_len() {
    ...
    let mut dst = vec![0u8; 16];
    let src = vec![0u8; 16];
    x86::proven::bytes::xor_rows_sse2(v2, &mut dst, &src, 0);
}
...
fn proven_xor_gather_rejects_out_of_bounds_offset() {
    ...
    let mut dst = vec![0u8; 32];
    x86::proven::bytes::xor_gather_avx2(v3, &region, &mut dst, &[40]);
}

#[test]
#[should_panic(expected = "not a whole number of 4-byte elements")]
fn proven_prime_rejects_partial_lane() {
    ...
    let src = vec![0u8; 7];
    x86::proven::prime::add_assign_m31_avx2(v3, &mut dst, &src);
}
        ...
        for &nterms in &[1usize, 3, 17] {
            ...
            for &lanes in &[2usize, 3, 4] {
                two_row_case(row_len, nterms, gf8b_coeff_at2, gf8b_ref, |want, terms| {
                    ...
                    x86::proven::gf8::matrix_overwrite2_8b(
                        token, &mut got, row_len, &terms, false, lanes,
                    );
                    assert_eq!(
                        got, want,
                ...
                two_row_case(row_len, nterms, gf8d_coeff_at2, gf8d_ref, |want, terms| {
                    ...
                    x86::proven::gf8::matrix_overwrite2_8d(
                        token, &mut got, row_len, &terms, false, lanes,
                    );
                    assert_eq!(
                        got, want,
            ...
            if row_len % 32 == 0 {
                two_row_case(row_len, nterms, gf8b_coeff_at2, gf8b_ref, |want, terms| {
                    ...
                    x86::proven::gf8::matrix_overwrite2_8b(token, rows, row_len, &terms, true, 4);
                    assert_eq!(&*rows, &want[..], "two-row nt 8b: len {row_len}");
                });
                two_row_case(row_len, nterms, gf8d_coeff_at2, gf8d_ref, |want, terms| {
                    let (mut backing, off) = aligned_noise(row_len * 2, 0xd7);
                    let rows = &mut backing[off..off + row_len * 2];
                    x86::proven::gf8::matrix_overwrite2_8d(token, rows, row_len, &terms, true, 4);
        ...
        for &nterms in &[1usize, 3, 17] {
            for &lanes in &[1usize, 2] {
                two_row_case(row_len, nterms, gf8b_coeff_at2, gf8b_ref, |want, terms| {
                    ...
                    x86::proven::gf8::matrix_overwrite2_shuffle_packed(
    ...
    for &row_len in &[32usize, 34, 128, 300] {
        for &nterms in &[1usize, 2, 9, 17] {
            six_row_case(row_len, nterms, gf8b_coeff_at2, gf8b_ref, |want, terms| {
                ...
                x86::proven::gf8::matrix_overwrite6_shuffle_8b(token, &mut got, row_len, &terms);
            ...
            six_row_case(row_len, nterms, gf8d_coeff_at2, gf8d_ref, |want, terms| {
                ...
                x86::proven::gf8::matrix_overwrite6_shuffle_8d(token, &mut got, row_len, &terms);
            ...
            six_row_case(row_len, nterms, gf8b_coeff_at2, gf8b_ref, |want, terms| {
                ...
                x86::proven::gf8::matrix_overwrite6_shuffle_packed_8b(
            ...
            six_row_case(row_len, nterms, gf8d_coeff_at2, gf8d_ref, |want, terms| {
                ...
                x86::proven::gf8::matrix_overwrite6_shuffle_packed_8d(
                    token, &mut got, row_len, &packed, &srcs,
                );
        ...
        for &sources in &[1usize, 2, 5, 33] {
            one_row_case(row_len, sources, |want, _coeffs, srcs, maps, terms| {
                for &lanes in &[3usize, 4] {
                    ...
                    x86::proven::gf8::matrix_overwrite1_tile_8d(
                        token, &mut got, row_len, &terms, lanes,
                    );
                    assert_eq!(got, want, "one-row tile: len {row_len} lanes {lanes}");

                    let mut got = noise(row_len, 0x7c2);
                    x86::proven::gf8::matrix_overwrite1_fullinit_8d(
                        token, &mut got, row_len, &terms, lanes,
                    );
                    ...
                    for &chunk in &[32usize, 96] {
                        ...
                        x86::proven::gf8::matrix_overwrite1_chunk_8d(
                ...
                if row_len % 32 == 0 {
                    for &lanes in &[3usize, 4] {
                        ...
                        x86::proven::gf8::matrix_overwrite1_external_8d(
                    ...
                    for &chunk in &[0usize, 64] {
                        ...
                        x86::proven::gf8::matrix_overwrite1_external_chunk_8d(
    ...
    for &nrows in &[1usize, 2, 3, 4, 6, 7] {
        for &sources in &[2usize, 33] {
            ...
            for &chunk in &[32usize, 96] {
                ...
                x86::proven::gf8::matrix_overwrite_chunk_8d(
            ...
            for &chunk in &[0usize, 64] {
                ...
                x86::proven::gf8::matrix_overwrite_external_grouped_8d(
...
fn proven_gf16_kernels_match_scalar() {
    ...
    check_mul_add(
        ...
        &coeffs,
        gf16_ref,
        |dst, c, src| x86::proven::gf16::mul_add_gfni(v3gfni, dst, TowerCoeff::new(c), src),
    );
    check_mul_assign(
        ...
        |dst, c| x86::proven::gf16::mul_assign_gfni(v3gfni, dst, TowerCoeff::new(c)),
    );
    check_mul_into(
        "gf16::mul_into_gfni",
        LENGTHS16,
        &coeffs,
        gf16_ref,
        |dst, c, src| x86::proven::gf16::mul_into_gfni(v3gfni, dst, TowerCoeff::new(c), src),
    );
    check_mul_add(
        "gf16::mul_add_avx2",
        LENGTHS16,
        &coeffs,
        gf16_ref,
        |dst, c, src| x86::proven::gf16::mul_add_avx2(v3, dst, &TowerTables::new(c), src),
    );
    check_mul_assign(
        ...
        &coeffs,
        scalar::mul_assign::<Gf16>,
        |dst, c| x86::proven::gf16::mul_assign_avx2(v3, dst, &TowerTables::new(c)),
    );
    check_mul_into(
        "gf16::mul_into_avx2",
        LENGTHS16,
        &coeffs,
        gf16_ref,
        |dst, c, src| x86::proven::gf16::mul_into_avx2(v3, dst, &TowerTables::new(c), src),
    );
    check_mul_add(
        "gf16::mul_add_ssse3",
        LENGTHS16,
        &coeffs,
        gf16_ref,
        |dst, c, src| x86::proven::gf16::mul_add_ssse3(v2, dst, &TowerTables::new(c), src),
    );
    check_mul_assign(
        ...
        &coeffs,
        scalar::mul_assign::<Gf16>,
        |dst, c| x86::proven::gf16::mul_assign_ssse3(v2, dst, &TowerTables::new(c)),
    );
    check_mul_into(
        "gf16::mul_into_ssse3",
        LENGTHS16,
        &coeffs,
        gf16_ref,
        |dst, c, src| x86::proven::gf16::mul_into_ssse3(v2, dst, &TowerTables::new(c), src),
    );

    check_elementwise::<Gf16>("gf16::elementwise_gfni", LENGTHS16, |dst, a, b| {
        x86::proven::gf16::elementwise_gfni(v3gfni, dst, a, b)
    });
    check_elementwise::<Gf16>("gf16::elementwise_avx2", LENGTHS16, |dst, a, b| {
        x86::proven::gf16::elementwise_avx2(v3, dst, a, b)
    });
    check_elementwise::<Gf16>("gf16::elementwise_ssse3", LENGTHS16, |dst, a, b| {
        x86::proven::gf16::elementwise_ssse3(v2, dst, a, b)
    });
    ...
    check_scatter(
        ...
        |rows, row_len, coeffs, src| {
            x86::proven::gf16::scatter_gfni(v3gfni, rows, row_len, coeffs, src)
        },
    );
    check_scatter(
        "gf16::scatter_avx2",
        ...
        |j| gf16::Elem::from_raw((j as u16).wrapping_mul(7411)),
        |rows, row_len, coeffs, src| {
            x86::proven::gf16::scatter_avx2(v3, rows, row_len, coeffs, src)
        },
    );
    check_scatter(
        "gf16::scatter_ssse3",
        ...
        |j| gf16::Elem::from_raw((j as u16).wrapping_mul(7411)),
        |rows, row_len, coeffs, src| {
            x86::proven::gf16::scatter_ssse3(v2, rows, row_len, coeffs, src)
        },
    );
    for &row_len in &[32usize, 130] {
        check_gather(
            ...
            |dst, coeffs, srcs| x86::proven::gf16::gather_gfni(v3gfni, dst, coeffs, srcs),
        );
        check_gather(
            "gf16::gather_avx2",
            row_len,
            gf16_ref,
            |i| gf16::Elem::from_raw((i as u16).wrapping_mul(613)),
            |dst, coeffs, srcs| x86::proven::gf16::gather_avx2(v3, dst, coeffs, srcs),
        );
        check_gather(
            "gf16::gather_ssse3",
            row_len,
            gf16_ref,
            |i| gf16::Elem::from_raw((i as u16).wrapping_mul(613)),
            |dst, coeffs, srcs| x86::proven::gf16::gather_ssse3(v2, dst, coeffs, srcs),
        );
    }
    check_matrix_accumulate(
        "gf16::matrix_gfni",
        ...
        gf16_ref,
        |rows, row_len, nrows, terms| {
            x86::proven::gf16::matrix_gfni(v3gfni, rows, row_len, nrows, terms)
        },
    );
    check_matrix_accumulate(
        "gf16::matrix_avx2",
        ...
        gf16_ref,
        |rows, row_len, nrows, terms| {
            x86::proven::gf16::matrix_avx2(v3, rows, row_len, nrows, terms)
        },
    );
    check_matrix_accumulate(
        "gf16::matrix_ssse3",
        ...
        gf16_ref,
        |rows, row_len, nrows, terms| {
            x86::proven::gf16::matrix_ssse3(v2, rows, row_len, nrows, terms)
        },
    );
}
...
fn proven_gf32_gf64_kernels_match_scalar() {
    ...
    check_mul_add(
        ...
        gf32_ref,
        |dst, c, src| {
            x86::proven::gf32::mul_add_gfni(token, dst, c, x86::proven::gf32_tiles(c), src)
        },
    );
    check_mul_assign(
        ...
        |dst, c| x86::proven::gf32::mul_assign_gfni(token, dst, c, x86::proven::gf32_tiles(c)),
    );
    check_mul_into(
        "gf32::mul_into_gfni",
        LENGTHS32,
        ...
        gf32_ref,
        |dst, c, src| {
            x86::proven::gf32::mul_into_gfni(token, dst, c, x86::proven::gf32_tiles(c), src)
        },
    );
    ...
    check_mul_add(
        ...
        gf64_ref,
        |dst, c, src| {
            x86::proven::gf64::mul_add_gfni(token, dst, c, x86::proven::gf64_tiles(c), src)
        },
    );
    check_mul_assign(
        ...
        |dst, c| x86::proven::gf64::mul_assign_gfni(token, dst, c, x86::proven::gf64_tiles(c)),
    );
    check_mul_into(
        "gf64::mul_into_gfni",
        LENGTHS64,
        ...
        gf64_ref,
        |dst, c, src| {
            x86::proven::gf64::mul_into_gfni(token, dst, c, x86::proven::gf64_tiles(c), src)
        },
    );
}
...
fn proven_fan_paar_kernels_match_scalar() {
    ...
    check_mul_add(
        ...
        &fp16_coeffs,
        fp16_ref,
        |dst, c, src| x86::proven::fan_paar::mul_add_avx2(v3, dst, &FpTowerTables::new(c), src),
    );
    check_mul_assign(
        "fan_paar::mul_assign_avx2",
        LENGTHS16,
        &fp16_coeffs,
        scalar::mul_assign::<FanPaar16>,
        |dst, c| x86::proven::fan_paar::mul_assign_avx2(v3, dst, &FpTowerTables::new(c)),
    );
    check_mul_into(
        ...
        &fp16_coeffs,
        fp16_ref,
        |dst, c, src| x86::proven::fan_paar::mul_into_avx2(v3, dst, &FpTowerTables::new(c), src),
    );
    check_mul_add(
        ...
        &fp16_coeffs,
        fp16_ref,
        |dst, c, src| x86::proven::fan_paar::mul_add_ssse3(v2, dst, &FpTowerTables::new(c), src),
    );
    check_mul_assign(
        ...
        &fp16_coeffs,
        scalar::mul_assign::<FanPaar16>,
        |dst, c| x86::proven::fan_paar::mul_assign_ssse3(v2, dst, &FpTowerTables::new(c)),
    );
    check_mul_into(
        ...
        &fp16_coeffs,
        fp16_ref,
        |dst, c, src| x86::proven::fan_paar::mul_into_ssse3(v2, dst, &FpTowerTables::new(c), src),
    );
    ...
    check_mul_add(
        ...
        &fp32_coeffs,
        fp32_ref,
        |dst, c, src| x86::proven::fan_paar::mul_add_fp32_avx2(v3, dst, c, src),
    );
    check_mul_assign(
        "fan_paar::mul_assign_fp32_avx2",
        LENGTHS32,
        &fp32_coeffs,
        scalar::mul_assign::<FanPaar32>,
        |dst, c| x86::proven::fan_paar::mul_assign_fp32_avx2(v3, dst, c),
    );
    check_mul_into(
        ...
        &fp32_coeffs,
        fp32_ref,
        |dst, c, src| x86::proven::fan_paar::mul_into_fp32_avx2(v3, dst, c, src),
    );
    ...
    check_mul_add(
        ...
        &fp64_coeffs,
        fp64_ref,
        |dst, c, src| x86::proven::fan_paar::mul_add_fp64_avx2(v3, dst, c, src),
    );
    check_mul_assign(
        "fan_paar::mul_assign_fp64_avx2",
        LENGTHS64,
        &fp64_coeffs,
        scalar::mul_assign::<FanPaar64>,
        |dst, c| x86::proven::fan_paar::mul_assign_fp64_avx2(v3, dst, c),
    );
    check_mul_into(
        ...
        &fp64_coeffs,
        fp64_ref,
        |dst, c, src| x86::proven::fan_paar::mul_into_fp64_avx2(v3, dst, c, src),
    );
}
...
fn proven_prime_kernels_match_scalar() {
    ...
    for &n in LANES {
        ...
        for (name, kernel, sub) in [
            (
                ...
                Box::new(|dst: &mut [u8], src: &[u8]| {
                    x86::proven::prime::add_assign_m31_avx2(v3, dst, src)
                }) as BinOp,
                false,
            ),
            (
                "sub_assign_m31_avx2",
                Box::new(|dst: &mut [u8], src: &[u8]| {
                    x86::proven::prime::sub_assign_m31_avx2(v3, dst, src)
                }),
                true,
            ),
            (
                "add_assign_m31_sse41",
                Box::new(|dst: &mut [u8], src: &[u8]| {
                    x86::proven::prime::add_assign_m31_sse41(v2, dst, src)
                }),
                false,
            ),
            (
                "sub_assign_m31_sse41",
                Box::new(|dst: &mut [u8], src: &[u8]| {
                    x86::proven::prime::sub_assign_m31_sse41(v2, dst, src)
                }),
                true,
            ),
        ] {
        ...
        let mut want = got.clone();
        x86::proven::prime::mul_add_m31_avx2(v3, &mut got, coeff_raw, &src);
        m31_ref(&mut want, coeff, &src);
        assert_eq!(got, want, "mul_add_m31_avx2: lanes {n}");

        let mut got = m31_lanes(n, 0x94);
        let mut want = got.clone();
        x86::proven::prime::mul_add_m31_sse41(v2, &mut got, coeff_raw, &src);
        m31_ref(&mut want, coeff, &src);
        assert_eq!(got, want, "mul_add_m31_sse41: lanes {n}");

        let mut got = m31_lanes(n, 0x95);
        let mut want = got.clone();
        x86::proven::prime::mul_assign_m31_avx2(v3, &mut got, coeff_raw);
        ...
        x86::proven::prime::mul_into_m31_avx2(v3, &mut got, coeff_raw, &src);
        m31_ref(&mut want, coeff, &src);
        assert_eq!(got, want, "mul_into_m31_avx2: lanes {n}");

        let mut got = m31_lanes(n, 0x97);
        let mut want = vec![0u8; n * 4];
        x86::proven::prime::mul_into_m31_sse41(v2, &mut got, coeff_raw, &src);
        m31_ref(&mut want, coeff, &src);
        assert_eq!(got, want, "mul_into_m31_sse41: lanes {n}");
        ...
        let mut want = got.clone();
        x86::proven::prime::mul_elementwise_m31_avx2(v3, &mut got, &a, &b);
        ...
        let mut want = got.clone();
        x86::proven::prime::mul_elementwise_m31_sse41(v2, &mut got, &a, &b);
        scalar::mul_elementwise::<Mersenne31>(&mut want, &a, &b);
        assert_eq!(got, want, "mul_elementwise_m31_sse41: lanes {n}");
    }
    ...
    for &n in &[0usize, 1, 3, 8, 9, 32] {
        ...
        for (name, kernel, sub) in [
            (
                "add_assign_gld_avx2",
                Box::new(|dst: &mut [u8], src: &[u8]| {
                    x86::proven::prime::add_assign_gld_avx2(v3, dst, src)
                }) as BinOp,
                false,
            ),
            (
                "sub_assign_gld_avx2",
                Box::new(|dst: &mut [u8], src: &[u8]| {
                    x86::proven::prime::sub_assign_gld_avx2(v3, dst, src)
                }),
                true,
            ),
            (
                "add_assign_gld_sse41",
                Box::new(|dst: &mut [u8], src: &[u8]| {
                    x86::proven::prime::add_assign_gld_sse41(v2, dst, src)
                }),
                false,
            ),
            (
                "sub_assign_gld_sse41",
                Box::new(|dst: &mut [u8], src: &[u8]| {
                    x86::proven::prime::sub_assign_gld_sse41(v2, dst, src)
                }),
                true,
            ),
        ] {
        ...
        let mut want = got.clone();
        x86::proven::prime::mul_add_gld_avx2(v3, &mut got, coeff_raw, &src);
        gld_ref(&mut want, gld_coeff, &src);
        assert_eq!(got, want, "mul_add_gld_avx2: lanes {n}");

        let mut got = gld_lanes(n, 0xa4);
        let mut want = got.clone();
        x86::proven::prime::mul_add_gld_sse41(v2, &mut got, coeff_raw, &src);
        gld_ref(&mut want, gld_coeff, &src);
        assert_eq!(got, want, "mul_add_gld_sse41: lanes {n}");

        let mut got = gld_lanes(n, 0xa5);
        let mut want = got.clone();
        x86::proven::prime::mul_assign_gld_sse41(v2, &mut got, coeff_raw);
        scalar::mul_assign::<Goldilocks>(&mut want, gld_coeff);
        assert_eq!(got, want, "mul_assign_gld_sse41: lanes {n}");

        let mut got = gld_lanes(n, 0xa6);
        let mut want = vec![0u8; n * 8];
        x86::proven::prime::mul_into_gld_avx2(v3, &mut got, coeff_raw, &src);
        gld_ref(&mut want, gld_coeff, &src);
        ...
        let mut want = got.clone();
        x86::proven::prime::mul_elementwise_gld_avx2(v3, &mut got, &a, &b);
        ...
        let mut want = got.clone();
        x86::proven::prime::mul_elementwise_gld_sse41(v2, &mut got, &a, &b);
        scalar::mul_elementwise::<Goldilocks>(&mut want, &a, &b);
        assert_eq!(got, want, "mul_elementwise_gld_sse41: lanes {n}");
    }
}
...
fn proven_avx512_kernels_match_scalar_where_summonable() {
    ...
    let mut want = got.clone();
    x86::proven::avx512::gf8_scatter(v4x, &mut got, row_len, &coeffs, &src);
    for (row, &coeff) in want.chunks_exact_mut(row_len).zip(&coeffs) {
        gf8b_ref(row, coeff, &src);
    }
    assert_eq!(got, want, "avx512::gf8_scatter");
    ...
    let mut want = got.clone();
    x86::proven::avx512::gf8_gather(v4x, &mut got, &gcoeffs, &srcs);
    for (&coeff, &src) in gcoeffs.iter().zip(&srcs) {
        gf8b_ref(&mut want, coeff, src);
    }
    assert_eq!(got, want, "avx512::gf8_gather");
    ...
    let mut want = got.clone();
    x86::proven::avx512::gf8_matrix(v4x, &mut got, row_len, nrows, &terms);
    fold_terms(&mut want, row_len, &terms, &gf8b_ref);
    assert_eq!(got, want, "avx512::gf8_matrix");
    ...
    x86::proven::avx512::gf16_mul_add(v4x, &mut got, TowerCoeff::new(coeff), &src);
