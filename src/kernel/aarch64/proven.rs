...
pub mod gf8 {
    ...
    pub fn mul_add_neon(
        ...
        selected::mul_add_neon(dst, table, src);
    }
    ...
    /// `dst = coeff * src`, fused single pass over NEON lanes.
    ///
    /// # Panics
    /// Panics if the slices differ in length.
    pub fn mul_into_neon(
        ...
        selected::mul_into_neon(dst, table, src);
    }
    ...
    /// Many sources into one row, destination tile in registers.
    ///
    /// # Panics
    ...
    pub fn matrix_neon(
        ...
        terms_geometry("gf8::matrix_neon", row_len, nrows, terms);
    ...
    /// `dst[i] = a[i] * b[i]` using the optional crypto extension (PMULL).
    ///
    /// # Panics
